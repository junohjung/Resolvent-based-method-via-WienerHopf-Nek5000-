function [DATA,RAWDATA]=readData(ts,folder,iy,ia,iz,actuatorDataFrom,computeTargetsCSD)
    ...
    for i=1:ny
        data_temp = dlmread(sprintf('%s/sens%1.0f/ProjShapes_01.dat',folder,iy(i)),'',1,0);
    ...
    if strcmp(actuatorDataFrom,'direct')
        for j=1:na
            data_temp  = dlmread(sprintf('%s/act%1.0f/ProjShapes_01.dat',folder,ia(j)),'',1,0);
            for i=1:ny
    ...
    elseif  strcmp(actuatorDataFrom,'adjoint')
        ...
        for i=1:nz
            data_temp    = dlmread(sprintf('%s/sens%1.0f/ProjShapes_01.dat',folder,iz(i)),'',1,0);
            for j=1:na
